...
def chat_generate_question(subject, context):
    ...
    thread = client.beta.threads.create(
        messages=[
            {
                "role": "user",
                "content": f'Give me a new problem for a learner in the subject: {subject}. They had the following note: {context}. They will not see your response, so do not repeat it later.'
            }
        ]
    )
